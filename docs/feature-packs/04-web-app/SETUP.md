...
| `~/.coodra/data.db` | populated by `coodra init` | Solo-mode primary store |
| Coodra CLI installed | `@coodra/cli` | For `init`/`start`/`pause` commands the web wraps |
...
### 1. Initialise Coodra in your project

```sh
cd /path/to/your-project
coodra init --project-slug my-project --no-graphify --ide claude
```

This writes:
- `~/.coodra/data.db` (SQLite primary store with the 11-table schema migrated to head)
- `~/.coodra/{logs,pids,config.json}` (operator state)
- `<cwd>/.coodra.json` (project sidecar pointing at `my-project`)
...
This launches the daemons (`hooks-bridge` on `:3101`, `mcp-server` on `:3100`) under launchd / systemd. Verify with `coodra doctor` — every essential check should be green.
...
The repo root `.env` has all the env vars `apps/web` needs (it was provisioned by `coodra init` plus the Module 04 additions). The copy step is required because Next.js doesn't read parent-directory `.env` files automatically.
...
```sh
pnpm --filter @coodra/web build
COODRA_MODE=team pnpm --filter @coodra/web start
```
...
DATABASE_URL='postgresql://...' \
  COODRA_MODE=team \
  pnpm --filter @coodra/sync-daemon start
```

Or wire it under launchd/systemd alongside `coodra start`. Logs land in `~/.coodra/logs/sync-daemon.log` (when started under the supervisor).

Verify the puller is alive: pause a kill-switch from the web admin (`/kill-switches`) and watch every developer's `~/.coodra/data.db` get the row within ~10s:

```sh
sqlite3 ~/.coodra/data.db "SELECT id, scope, target, mode, paused_by_session_id FROM kill_switches WHERE resumed_at IS NULL;"
```
...
- **Railway** — set `apps/web` as the service root with `pnpm --filter @coodra/web start` as the start command.
...
pnpm --filter @coodra/web test:unit            # 27/27 expected
pnpm --filter @coodra/web build
...
LIVE_SUPABASE_TEST=1 CLERK_LIVE_TEST=1 \
  ...
  pnpm --filter @coodra/web test:integration   # 7/7 expected
```
...
```sh
# Start in solo mode
COODRA_MODE=solo pnpm --filter @coodra/web start &
sleep 4
...
| `/packs` shows empty list when packs exist | `process.cwd()` is `apps/web/` not the repo root | Already fixed (S7) — walks up from cwd; or set `COODRA_PACKS_ROOT` env var |
...
For deeper issues, check `~/.coodra/logs/{hooks-bridge,mcp-server,sync-daemon}.log` and the `coodra doctor --full --json` output.

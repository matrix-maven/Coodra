...
## 12. Pre-M04 fix-ups PR (lands BEFORE S1 — first item is the live `.strict()` blocker)

OQ-6 lock = (b) — pre-M04 fix-ups land as a separate PR before any M04 implementation slice. Per the project lead's directive (2026-05-03): fix the `.strict()` bug first, because it was observed firing live in this very session (the Stop hook returned the PreToolUse shape and the bridge rejected it with `invalid_hook_payload`).

**Branch:** `fix/pre-m04-blockers` (off `main`).

**Slices on the fix-up PR (in order, this order is locked):**

1. **`.strict()` → `.passthrough()` on every adapter payload schema.** Files: `packages/shared/src/hooks/payloads/{claude-code,windsurf,cursor}.ts`. Per `context_memory/blockers.md:98-125`, swap `.strict()` for `.passthrough()`. The adapter projects only known fields into `HookEvent`, so passthrough is safe. **Live verified failing in this session 2026-05-03 — Stop hook returning PreToolUse shape was rejected with `invalid_hook_payload` instead of routed.** Add an integration test at `apps/hooks-bridge/__tests__/integration/passthrough-tolerance.test.ts` that POSTs Claude Code's actual SessionStart payload (with `transcript_path` + `source`) and asserts `additionalContext` is returned, AND the same shape mis-tagged as Stop is routed correctly.

2. **`init` seeds zero policy rules — ship a default-deny seed.** Per `context_memory/blockers.md:127-148`. `commands/init.ts` currently calls `ensureGlobalProject` + `ensureProject` but never inserts policy rules. Ship a default-deny fixture at `packages/db/drizzle/seeds/default-deny.json` containing the universal-safe denies (per blockers.md): Write to `**/.env.production` + `**/.env.*.production`; Bash for `rm -rf /`-shaped commands; Write to `.git/**`. Init applies them after migrations.

3. **`seedFeaturePack` writes only spec.md — fix mcp-server's `get_feature_pack` ENOENT.** Per `context_memory/blockers.md:150-172`. Two equivalent fixes; pick (b) — soften `apps/mcp-server/src/lib/feature-pack.ts::readPackFromDisk` to tolerate missing `implementation.md` / `techstack.md` the same way `feature-pack-loader.ts` does. One rule for both readers; cleaner architecture. Add a unit test that the MCP `get_feature_pack` tool returns a populated response when only spec.md exists, and an integration test that runs the full flow against a freshly-init'd project.

**Acceptance for the fix-up PR:** all three fixes ship in one PR, each as its own commit. Lint + typecheck + unit + integration all pass. Two new integration tests (passthrough + post-init `get_feature_pack`). Manual verification: spin up a fresh `contextos init` against a sandbox CONTEXTOS_HOME, fire Claude Code's actual SessionStart wire payload, observe `additionalContext` returned and `additionalContext` NOT empty (because the default-deny seed populated `policies` so `query_codebase_graph` and friends have something to ground against — wait, that's wrong — additionalContext is from feature-pack, not policies). Restate manual verification in the fix-up PR description.

**Why this order:** `.strict()` is observable user-impacting behaviour right now (bridge silently fails open on every real Claude Code session). The other two are only observable on a stranger install. So `.strict()` is a hot fix; the other two are cleanup that happens in the same PR because they're also blockers and shipping them separately would be three small PRs.

**M04 S1 does not start until this PR is merged to main.** S1's first acceptance check is "fresh `contextos init`'s SessionStart hook against the bridge returns a populated `additionalContext`" — that check requires the `.strict()` fix.
...
| OQ-6 | (b) Pre-M04 fix-ups land as a separate PR before S1; `.strict()` is item one | `.strict()` was observed firing live this session; fix-up PR de-risks S1 acceptance | §12, separate `fix/pre-m04-blockers` PR (lands first), then S1 |

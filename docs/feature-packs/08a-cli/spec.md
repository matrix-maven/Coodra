...
|---|---|---|
| `coodra init [--project-slug <slug>] [--ide <claude\|cursor\|both>] [--no-graphify] [--dry-run] [--force]` | Creates `~/.coodra/{data.db,.env,logs/,pids/}` plus project-local `.coodra/config.json`; runs auto-migrate against the local SQLite (and `ensureGlobalProject` per F7); writes/merges `.mcp.json`; creates `docs/feature-packs/<slug>/` with seeded `meta.json` + `spec.md` skeleton; optionally invokes Graphify. Coodra runtime values such as `LOCAL_HOOK_SECRET`, `MCP_SERVER_PORT`, and `HOOKS_BRIDGE_PORT` live in `~/.coodra/.env`; init does not create or modify the project's application `.env` or root `.coodra.json`. Re-run is idempotent by default and leaves user-owned files alone. `--force` overwrites Coodra-owned baselines. See §11 Decision 3. | 0 ok / 1 detection-failed / 2 user-input-needed / 3 file-collision-needs-`--force` |
...
| `<repo>/.coodra/config.json` | Bridge's `projectSlugResolver` config | `{ "version": 1, "projectSlug": "<derived or --project-slug>" }` |
...
| 12 | Project registered for `cwd` — `.coodra/config.json` parses, slug resolves to a `projects` row | yellow | Pre-condition for governance — without this, the bridge falls to `__global__` (still audited per F7 but project-specific rules don't fire). |
...
- **Decision:** Re-run inspects each Coodra-owned file (`.coodra/config.json`, `.mcp.json`, `docs/feature-packs/<slug>/`) plus Coodra home runtime state, leaves it alone if already correct, merges only if drift is detected, and never destroys user edits. `--force` overrides Coodra-owned baselines. Re-run with no flags is always safe.

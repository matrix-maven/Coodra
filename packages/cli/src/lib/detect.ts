 ...
 * output. Coodra supports native Claude Code, Codex, Cursor, Devin, and
 * Antigravity plugins.
 */
export type IDE = 'claude' | 'codex' | 'cursor' | 'devin' | 'antigravity';
 ...
 */
export const IDE_ORDER: readonly IDE[] = ['claude', 'codex', 'cursor', 'devin', 'antigravity'] as const;
...
export function resolveIdeSelection(input: ResolveIdeSelectionInput): ResolveIdeSelectionResult {
  ...
  if (tokens.length === 0) {
    return {
      ok: false,
      error:
        '--ide value is empty. Pass one of: claude, codex, cursor, devin, antigravity, all (comma-separated for multiple).',
    };
  }
  ...
  for (const token of tokens) {
    if (!allowed.has(token)) {
      return {
        ok: false,
        error: `--ide: unknown agent '${token}'. Valid: claude, codex, cursor, devin, antigravity, all.`,
      };
    }
 ...
 * order matches the candidate list (Claude, Codex, Cursor, Devin,
 * Antigravity). An empty array means no supported IDE is installed —
 * `init` warns the user.
 *
 * Detection dirs:
 *   - claude      → ~/.claude
 ...
 *   - cursor      → ~/.cursor
 *   - devin       → ~/.devin   (a general "the app touched this machine" signal —
 *                   confirmed present after running the bundled Devin CLI once;
 *                   same home-relative dot-dir convention as the other three,
 *                   rather than Devin's own XDG `~/.config/devin/`)
 *   - antigravity → ~/.gemini/antigravity — deliberately NOT the bare
 *                   `~/.gemini`, which could plausibly also exist for Google's
 *                   separate plain "Gemini CLI" product without Antigravity
 *                   being installed at all. `~/.gemini/antigravity/` is
 *                   confirmed (on a real install) to be Antigravity's own
 *                   subdirectory, avoiding that false-positive.
 */
export async function detectIDE(deps: DetectionDeps = {}): Promise<IDE[]> {
  ...
  const candidates: Array<{ ide: IDE; dir: string }> = [
    ...
    { ide: 'devin', dir: '.devin' },
    { ide: 'antigravity', dir: join('.gemini', 'antigravity') },
  ];
  const found: IDE[] = [];

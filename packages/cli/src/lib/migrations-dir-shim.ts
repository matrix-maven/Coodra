...
// artifact (`@coodra/contextos-cli/dist/runtime/...`), set
// `CONTEXTOS_MIGRATIONS_DIR` to the bundled drizzle path BEFORE
// `@coodra/contextos-db`'s module-level `MIGRATIONS_FOLDER` constant
...
// index.ts` so every downstream `import '@coodra/contextos-db'` sees the
// right env.
//
...
if (sqlite !== null && typeof process.env.CONTEXTOS_MIGRATIONS_DIR !== 'string') {
  // bundledMigrationsDir returns the dialect-specific subfolder
  // (`<...>/sqlite`); strip the suffix because
  // `@coodra/contextos-db::MIGRATIONS_FOLDER` re-appends the dialect.
  process.env.CONTEXTOS_MIGRATIONS_DIR = sqlite.replace(/[\\/]+sqlite$/, '');
}

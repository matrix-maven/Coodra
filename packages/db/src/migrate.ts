 ...
 *
 * Bundled-deploy override: when `@coodra/db` is inlined into the
 * `@coodra/cli` esbuild bundle (decision `dec_83ba10c1`,
 ...
 *
 *   - `COODRA_MIGRATIONS_DIR_SQLITE`     → MIGRATIONS_FOLDER.sqlite
 *   - `COODRA_MIGRATIONS_DIR_POSTGRES`   → MIGRATIONS_FOLDER.postgres
 *   - `COODRA_MIGRATIONS_DIR`            → both, suffixed by dialect
 *     (e.g. `<dir>/sqlite` and `<dir>/postgres`).
 */
function resolveMigrationsFolderFromEnv(dialect: 'sqlite' | 'postgres', fallback: string): string {
  const dialectKey = dialect === 'sqlite' ? 'COODRA_MIGRATIONS_DIR_SQLITE' : 'COODRA_MIGRATIONS_DIR_POSTGRES';
  ...
  const baseOverride = process.env.COODRA_MIGRATIONS_DIR;

...
/**
 * `packages/lifecycle/src/capture-base-sha.ts` — Module 06 (Run Diff,
 * 2026-05-09). Spawns `git rev-parse HEAD` in the project's cwd at
 * SessionStart and persists the result in `runs.base_sha`.
 *
 * Moved here from `apps/hooks-bridge/src/lib/capture-base-sha.ts`
 * (COOD-60, 2026-08-09) — same extraction as `finalizeRunOnSessionEnd`:
 * the HTTP Hooks Bridge's own SessionStart handler still calls this via
 * the re-export shim left at its old path, and the native
 * `lifecycle_event` MCP tool (the only SessionStart path for native
 * plugins since COOD-53 retired the bridge from the runtime path) now
 * calls it too. Before this move, `runs.base_sha` was only ever
 * populated for the retired bridge transport — every native-plugin
 * session's run-diff landed `error='no_base_sha'` regardless of
 * whether the project was actually a git repo.
 *
 * Why this is a separate module and not inlined in a handler:
...
const captureLogger = createLogger('lifecycle.capture-base-sha');

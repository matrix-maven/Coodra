...
/**
 * `packages/lifecycle/src/features-index-loader` — reads
 * `<cwd>/.coodra/recipes/INDEX.json` for SessionStart `additionalContext`
 * injection.
 *
 * Moved here from `apps/hooks-bridge` (COOD-63, 2026-08-09). COOD-53
 * routed every native plugin's SessionStart through the `lifecycle_event`
 * MCP tool, which had no equivalent — so agents had no way to discover
 * available Agent Recipes at session start unless they thought to call
 * `list_recipes` unprompted. The `coodra-recipe` skill and the
...
const featuresIndexLoaderLogger = createLogger('lifecycle.features-index-loader');

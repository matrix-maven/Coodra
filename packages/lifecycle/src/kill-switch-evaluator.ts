...
/**
 * `packages/lifecycle/src/kill-switch-evaluator` — short-circuit
 ...
 *
 * Moved here from `apps/hooks-bridge/src/lib/kill-switch-evaluator.ts`
 * (COOD-61, 2026-08-09) — same extraction as `captureBaseSha`/
 * `finalizeRunOnSessionEnd`. COOD-53 routed every native plugin's
 * PreToolUse through `apps/mcp-server`'s `lifecycle_event` tool, which
 * never consulted this evaluator — so `coodra pause` was silently
 * non-functional for every supported agent (Claude Code, Codex,
 * Cursor, Devin, Antigravity). The bridge keeps importing it via the
 * re-export shim left at its old path; the native handler now calls it
 * too, so a single implementation governs both transports.
 *
...
const killSwitchEvaluatorLogger = createLogger('lifecycle.kill-switch-evaluator');
